    ...
    updatingTheBlog(formData, token, router.query.slug).then((data) => {
      if (data.error) {
        ...
        setInfos({
          ...infos,
          title: "",
          success: `${data.title} is successfully edited `,
        });
    ...
    return (
      ...
      categories.map((cat, index) => (
        <li key={index} className="list-unstyled">
          <input
            onChange={handleCategoriesToggle(cat._id)}
            checked={searchSkimCategory(cat._id)}
            type="checkbox"
            className="mr-2"
          />
  ...
  const displayTaglists = () => {
    return (
      ...
      taglists.map((tagg, index) => (
        <li key={index} className="list-unstyled">
          <input
            onChange={handleTaglistsToggle(tagg._id)}
            checked={searchSkimTag(tagg._id)}
            type="checkbox"
            className="mr-2"
          />
